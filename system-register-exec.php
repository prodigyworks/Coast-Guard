<?php
	...
	$sex = clean($_POST['sex']);
	$certified = clean($_POST['certified']);
	...
	if (! isset($_GET['id'])) {
		...
		$qry = "INSERT INTO {$_SESSION['DB_PREFIX']}members " .
				"(firstname, lastname, fullname, login, passwd, email, imageid, accepted, guid, status, certified, sex, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) " .
				"VALUES" .
				"('$fname','$lname', '$fullname', '$login', '".md5($_POST['password'])."', '$email', $imageid, 'Y', '$guid', 'Y', '$certified', '$sex', NOW(), " . getLoggedOnMemberID() . ", NOW(), " .  getLoggedOnMemberID() . ")";
		$result = @mysql_query($qry);
